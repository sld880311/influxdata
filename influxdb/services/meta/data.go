...
/**
	Databases：管理RetentionPolicies和ContinuousQueries
			Name                   string
			DefaultRetentionPolicy string
			# 每个DB都有RP，RP规定了每两个shardgroup之间的时间跨度ShardGroupDuration
			# 即每过一个ShardGroupDuration就会生产切换到下一个新的shardgroup中
			RetentionPolicies      []RetentionPolicyInfo：封装了数据库使用的策略信息
					Name               string
					ReplicaN           int：副本个数
					Duration           time.Duration：数据保留时长
					ShardGroupDuration time.Duration：shardgroup切分时长
					ShardGroups        []ShardGroupInfo：当前节点的所有shardgroup信息
							ID          uint64
							StartTime   time.Time：shardgroup中最早时间
							EndTime     time.Time：shardgroup中最晚时间
							DeletedAt   time.Time
							# shardgroup中包含所有的shard，对于同一个shardgroup按照series key（point key）不同散列到不同的shard中
							Shards      []ShardInfo：定义了shard的id和它位于的data node
								ID     uint64
								Owners []ShardOwner
									NodeID uint64
			...
			ContinuousQueries      []ContinuousQueryInfo
					Name  string
					Query string
	Users：封装用户的信息（用户名和密码）以及对db的操作权限
		// User's name.
		Name string
		// Hashed password.
		Hash string
