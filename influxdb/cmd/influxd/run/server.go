...
func NewServer(c *Config, buildInfo *BuildInfo) (*Server, error) {
	...
	s := &Server{
		...
		/**
			创建meta：services/meta/client.go
			初始化ClusterID和index
		 */
	...
	/**
		启动meta
		1.加载meta.db中的磁盘文件到内存中（利用protocol buffer反序列化）
		2.将meta数据写入磁盘，所有的meta信息都有对应的protocol buffer结构，依赖protocol buffer做序列化和反序列化。
	 */
...
func (s *Server) Open() error {
	...
	// Append services.添加各种service
	s.appendMonitorService()
	// 预创建shardgroup
	s.appendPrecreatorService(s.config.Precreator)
	// 使用tcp连接复用器，处理snapshot相关请求
	s.appendSnapshotterService()
	// 连续query服务
	s.appendContinuousQueryService(s.config.ContinuousQuery)
	// http服务，接收并处理所有客户端的请求
	s.appendHTTPDService(s.config.HTTPD)
	// 依据retentionpolicy周期性的清理数据
	s.appendRetentionPolicyService(s.config.Retention)
	// 追加graphite、collectd，opentsdb对tsdb格式的支持
