...
func reloadLoop(
	...
	for <-reload {
		...
		/**
		可以使用systemctl reload telegraf.service，重新加载配置文件
		原理是：使用命令/bin/kill -HUP pid完成（挂起进程）会触发缓存数据flush
		[agent] Hang on, flushing any cached metrics before shutdown
		SIGHUP：挂起
		SIGTERM：kill不加参数，当前进程收到信号，子进程没有；如果当前进程被kill，子进程的父进程就会被init，也就是pid=1的进程
		SIGINT：ctrl+c终止，信号被当前进程树接收到，当前进程和子进程都会收到信号
		 */
...
func main() {
	...
	通过浏览器查看：http://localhost:6060/debug/pprof/
	 */
	if *pprofAddr != "" {
		go func() {
	...
	/**
	telegraf version ：打印telegraf的版本信息
	telegraf config：打印telegraf的配置信息
	 */
	if len(args) > 0 {
		switch args[0] {
		case "version":
